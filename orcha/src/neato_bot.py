...
class neato_bot():
    def __init__(self,ip,token,rest_port=7061,streaming_port_range=[6000,10000],hostname=None,rosmaster_uri=""):
        self.ip=ip
        self.token=token
        ...
        self.cmd=""
        log.info("New neato_bot() Created! rosmaster_uri:{0} ip:{1}".format(self.rosmaster_uri,self.ip))
    ...
    def start_streaming(self):
        ...
        self.cmd="./stream-server -incoming {ffmpeg_stream_port} -readbuffer {readbuffer_port} -secret {secret} -websocket {ws_port} -writebuffer {writebuffer_port}".format(
            ffmpeg_stream_port=ffmpeg_stream_port,ws_port=ws_port,writebuffer_port=writebuffer_port,readbuffer_port=readbuffer_port,secret=self.token)
        ...
        log.info("{0}:{1} is starting the stream at {2} from {3}".format(self.token,self.ip,self.stream_url,self.stream2url))
    ...
    def stop_streaming(self):
        ...
        if self.cmd=="":
            log.error("call start_streaming First!")
            return 
        self.local_session_pool.kill_session_process(self.cmd)
        self.call_mini_rest_service("stop_streaming/{0}".format(self.stream2url))
        
        log.info("{0}:{1} has stopped  the stream at {2} from {3}".format(self.token,self.ip,self.stream_url,self.stream2url))
    ...
    def launch_telep(self):
        ...
        log.info("Publishing to /neato_{0}/cmd_vel".format(self.token))

        self.call_mini_rest_service("launch_telep/{0}/{1}".format(self.rosmaster_uri,self.token))
        log.info("Launching Telep Services on {0}:{1}".format(self.token,self.ip))
        return
    ...
    def kill_telep(self):
        ...
        self.pub_vel=None
        log.info("killing Telep Services on {0}:{1}".format(self.token,self.ip))
    
        return

    def call_mini_rest_service(self,serv):
        url="http://{0}:{1}/{2}".format(self.ip,self.rest_port,serv.replace("http://",""))
        log.debug("calling mini Rest with URL {0}".format(url))
        try:
            request=req.get(url)
            if request.__dict__["status_code"]==200:
                log.debug("good request,server says {0}".format(request.content))

            if request.__dict__["status_code"]!=200:
                log.warn("BAD request,server says {0}".format(request.content))
